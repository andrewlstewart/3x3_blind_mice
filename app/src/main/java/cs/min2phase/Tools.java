...
public class Tools {
    ...
    /**
     * Computes the "difference" cube state: diff = current⁻¹ * target.
     * The result is a facelet string representing the transformation needed
     ...
     * @return 54-char facelet string of the difference state
     */
    public static String computeDifferenceFacelets(String currentFacelets, String targetFacelets) {
        byte[] currentBytes = new byte[54];
        byte[] targetBytes = new byte[54];
        for (int i = 0; i < 54; i++) {
            currentBytes[i] = faceletCharToByte(currentFacelets.charAt(i));
            targetBytes[i] = faceletCharToByte(targetFacelets.charAt(i));
        }

        CubieCube currentCube = new CubieCube();
        CubieCube targetCube = new CubieCube();
        Util.toCubieCube(currentBytes, currentCube);
        Util.toCubieCube(targetBytes, targetCube);

        // Invert current
        currentCube.invCubieCube();

        // Compose: diff = currentInv * target
        CubieCube diffCube = new CubieCube();
        CubieCube.CornMult(currentCube, targetCube, diffCube);
        CubieCube.EdgeMult(currentCube, targetCube, diffCube);

        return Util.toFaceCube(diffCube);
    }

    private static byte faceletCharToByte(char c) {
        switch (c) {
            case 'U': return 0;
            case 'R': return 1;
            case 'F': return 2;
            ...
            case 'L': return 4;
            case 'B': return 5;
            default: throw new IllegalArgumentException("Invalid facelet char: " + c);
        }
    }
}
